...
class TestWeakRule:
    ...
    def test_invalid_label_rejected(self) -> None:
        ...
        with pytest.raises(ValueError, match="unknown label"):
            WeakRule(
                name="bad", field="app_id", pattern="com.x.Y", label="InvalidLabel"
            )
...
class TestMatchRule:
    ...
    def test_app_category_match(self) -> None:
        ...
        rules = [
            WeakRule(
                name="chat", field="app_category", pattern="chat", label="Communicate"
            )
        ]
...
class TestApplyWeakRules:
    def test_single_row_produces_one_span(self) -> None:
        ...
        assert spans[0].end_ts == _ts(10, 0) + dt.timedelta(
            seconds=DEFAULT_BUCKET_SECONDS
        )
    ...
    def test_consecutive_same_label_merged(self) -> None:
        ...
        rows = [
            _feature_row(_ts(10, i), app_id="com.microsoft.VSCode") for i in range(3)
        ]
        spans = apply_weak_rules(_features_df(rows), rules=rules)
        assert len(spans) == 1
        assert spans[0].start_ts == _ts(10, 0)
        assert spans[0].end_ts == _ts(10, 2) + dt.timedelta(
            seconds=DEFAULT_BUCKET_SECONDS
        )
...
class TestBuildDefaultRules:
    ...
    def test_all_labels_valid(self) -> None:
        ...
        for rule in build_default_rules():
            assert rule.label in LABEL_SET_V1, (
                f"Rule {rule.name} has invalid label {rule.label}"
            )
    ...
    def test_count_matches_maps(self) -> None:
        ...
        rules = build_default_rules()
        expected = (
            len(APP_ID_RULES) + len(APP_CATEGORY_RULES) + len(DOMAIN_CATEGORY_RULES)
        )
