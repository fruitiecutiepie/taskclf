...
**`inference_policy.json` is not auto-created on startup.** It is created only when you explicitly ask for it—for example **Edit Inference Policy** in the tray (which writes a starter if the file is missing), or `taskclf policy create` / `taskclf train tune-reject --write-policy`.
...
Authoritative API: [core.inference_policy](../api/core/inference_policy.md) (`render_default_inference_policy_template_json`, `write_inference_policy_starter_template`).

## What updates the live file

- **Edit Inference Policy** — creates a starter when missing (seeded from a resolved model bundle when possible; otherwise the placeholder with `_help`).
