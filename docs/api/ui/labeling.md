...
- **WebSocket** (`/ws/predictions`) streams live events from the ActivityMonitor:
  ...
  - `suggestion_cleared` -- published after successful label saves (via `POST /api/labels` or `POST /api/notification/accept`) and dismissals (`POST /api/notification/skip`): `reason` (e.g. `"label_saved"`, `"skipped"`). Clients clear the active suggestion on receipt. The frontend also applies a 10-minute TTL: if no new `suggest_label` event replaces it, the suggestion auto-dismisses.
