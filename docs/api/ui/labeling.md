...
- **Auto-save BreakIdle** -- When a completed activity block is detected as idle (lockscreen app was dominant, or the model suggested `BreakIdle`), the tray auto-saves the label with `provenance="auto_idle"` and publishes a `label_created` event. No user confirmation is required since the user was away. Known lockscreen processes: `loginwindow` (macOS), `lockapp.exe`/`logonui.exe` (Windows), `gnome-screensaver`, `gnome-shell`, `i3lock`, `swaylock`, `xscreensaver`, `light-locker`, `slock` (Linux).
