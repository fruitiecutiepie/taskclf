...
- **Today's Labels** -- tray menu action that shows a desktop notification summarizing today's labeling progress: total label count, total time, and per-label breakdown (e.g. "Today: 5 labels, 1h 15m -- Build 45m, Debug 20m, Write 10m"). Counts use the UTC calendar day. Also available via `GET /api/labels/stats` for programmatic access.
