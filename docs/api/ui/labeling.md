...
- **Edit Config** -- tray menu action that opens `config.json` in the default text editor. All runtime settings (`notifications_enabled`, `privacy_notifications`, `poll_seconds`, `transition_minutes`, `aw_host`, `title_salt`, `ui_port`) are persisted to this file on startup. Edited values take effect on next restart; explicit CLI flags always override config file values.
