...
- `user_config.template.toml` — **example** of the per-install `config.toml` written next to `data/processed` (tray/UI user settings). Canonical copy for review; runtime file is created on first run. See the [User config template](https://fruitiecutiepie.github.io/taskclf/guide/config_template/) guide.
