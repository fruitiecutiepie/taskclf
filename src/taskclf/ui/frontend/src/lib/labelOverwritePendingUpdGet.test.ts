...
describe("label_overwrite_pending_upd_get", () => {
  ...
  it("returns null when time has passed and conflicts no longer overlap", () => {
    ...
    const result = label_overwrite_pending_upd_get(
      pending,
      selection_base({ selectedMinutes: 5 }),
      now,
    );
  ...
  it("preserves conflicts that still overlap after time change", () => {
    ...
    const result = label_overwrite_pending_upd_get(
      pending,
      selection_base({ selectedMinutes: 15 }),
      now,
    );
  ...
  it("updates start and end to reflect current time", () => {
    ...
    const result = label_overwrite_pending_upd_get(
      pending,
      selection_base({ selectedMinutes: 10 }),
      now,
    );
    if (result === null) {
  ...
  it("uses lastLabelEndTs as start when fillFromLast is true", () => {
    ...
    const result = label_overwrite_pending_upd_get(
      pending,
      selection_base({ fillFromLast: true, lastLabelEndTs: iso(10, 45) }),
      now,
    );
    if (result === null) {
      throw new Error("expected non-null result");
  ...
  it("returns null when fillFromLast range does not overlap any conflict", () => {
    ...
    const result = label_overwrite_pending_upd_get(
      pending,
      selection_base({ fillFromLast: true, lastLabelEndTs: iso(10, 0) }),
      now,
    );
    expect(result).toBeNull();
  });
  ...
  it("forces extendForward when selectedMinutes is 0", () => {
    ...
    const result = label_overwrite_pending_upd_get(
      pending,
      selection_base({ selectedMinutes: 0, extendFwd: false }),
      now,
    );
    expect(result).not.toBeNull();
  ...
  it("uses extendFwd from selection when selectedMinutes > 0", () => {
    ...
    const result = label_overwrite_pending_upd_get(
      pending,
      selection_base({ selectedMinutes: 5, extendFwd: false }),
      now,
    );
    expect(result).not.toBeNull();
